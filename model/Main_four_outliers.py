...
if __name__ == '__main__':
    ...
    for k in [2]:
        ...
        for per in range(1, 6):
            ...
            Y_train_pro = RLSDSPCA_cal_projections(x_train, y_train, 1e-5, 1e4, 0.1, k)
    ...
    for k in [2]:
        ...
        for per in range(1, 6):
            x_train, x_test, y_train, y_test = train_test_split(X.T, B.T, test_size=33, random_state=per)
            Y_train_pro = SDSPCA_cal_projections(x_train, y_train, 1e3, 1e6, k)
            Y_train_pro = np.mat(Y_train_pro)
    ...
    for k in [2]:
        ...
        for per in range(1, 6):
            x_train, x_test, y_train, y_test = train_test_split(X.T, B.T, test_size=33, random_state=per)
            Y_train_pro = gLSPCA_cal_projections(x_train, 1e8, 1e7, k)
            Y_train_pro = np.mat(Y_train_pro)
