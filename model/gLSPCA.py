...
def gLSPCA_Algorithm(xMat,laplace,beta,garma,k,n):
    ...
    for m in range(0, 10):
        ...
        Z = -(xMat.T * xMat) + beta * vMat + garma * laplace  # (643, 643)
...
def gLSPCA_cal_projections(X_data,beta1, gamma1, k_d):
    # nclass = 2
    ...
    R = W_L
    M = cal_laplace(R)
    Y= gLSPCA_Algorithm(X_data.transpose(), M, beta1, gamma1, k_d, n)
    return Y